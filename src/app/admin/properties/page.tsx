  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-primary-900">Properties</h1>
          <p className="text-primary-700 text-sm mt-1">Manage all property listings on your website.</p>
        </div>
          ...
          <table className="w-full text-left text-sm text-primary-800">
            ...
            <tbody className="divide-y divide-primary-100">
              {properties.length > 0 ? properties.map((property) => (
                <tr key={property.id} className="hover:bg-primary-50/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-primary-900">{property.title}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${
                      property.status === 'Available' ? 'bg-primary-100 text-primary-800' :
